...
protected function index() {
  ...
  $this->load->model('checkout/order');
      ...
      if (isset($_SESSION['secret_key'])) {
          ...
          $site = $_SESSION['site'];
      $request_param =array('secret_key'=>$secret_key,'merchant_email'=>$paytabs_merchant,'payment_reference'=>$_POST['payment_reference']);
     ...
     $object = json_decode($response_data);
        ...
        $this->data['text_message'] = $object->result;

        $this->model_checkout_order->confirm($this->session->data['order_id'], 10, '', false);        
        $this->template = $this->config->get('config_template') . '/template/payment/paytabs_error.tpl';
  ...
  public function send() {
    ...
    $total = $this->currency->format($order_info['total'], $order_info['currency_code'], $order_info['currency_value'], false);
      ...
      $other_charges = $total - $this->cart->getSubTotal(); 
    ...
    if ($order_info['payment_iso_code_3'] != 'USA' or $order_info['payment_iso_code_3'] != 'CAN') {
      ...
      'secret_key'           => $this->config->get('paytabs_security'),
      ...
      'state'      			 =>	$order_info['payment_zone'] == '' ? $order_info['payment_zone'] : $order_info['payment_zone'],
      ...
      'CustomerId'           => $this->session->data['order_id'],
      'ProductName'          => $products_per_title,
      'ShippingMethod'       => 'Paytabs',
      'other_charges'        => $other_charges,
      'cms_with_version'     =>'OpenCart 1.5',
